...
import { saveWhiteboardToWalrus, loadWhiteboardFromWalrus } from './whiteboard-walrus.js';
import { saveBlobIdToSupabase, getBlobIdFromSupabase } from './supabase.js';

// Whiteboard class
export class Whiteboard {
    constructor(scene, wall, size = { width: 4, height: 3 }, position, username) {
        this.scene = scene;
        this.wall = wall;
        this.size = size;
    ...
    async init() {
        ...
        // Try to load whiteboard data from Walrus
        try {
            // Get blob ID from Supabase based on username
            const blobId = await getBlobIdFromSupabase(this.username);
            
            if (blobId) {
                // Load the whiteboard data from Walrus using the blob ID
                const dataUrl = await loadWhiteboardFromWalrus(blobId);
                if (dataUrl) {
                    // Load the image onto the canvas
                    await this.loadImageToCanvas(dataUrl);
                    console.log("Whiteboard loaded successfully from blob ID:", blobId);
                } else {
                    console.warn("Blob ID found but no data returned from Walrus");
                }
            } else {
                console.log("No whiteboard data found for this user");
            }
        } catch (error) {
    ...
    async saveToBlockchain() {
        ...
        try {
            ...
            if (supabaseResult.success) {
                ...
                this.showMessage("Whiteboard uploaded but failed to save reference");
                return false;
            }
        } catch {
            console.error("Error saving whiteboard:", error);
            this.showMessage("Failed to upload to Walrus, try again", blobId);
            return false;
        }
    }
}
...
export function initWhiteboard(visitorAuthSession, username, scene, camera, controls, roomSize) {
